...
return (
<div className="bg-white p-5 rounded-2xl shadow-sm border hover:shadow-lg transition">
<div className="mb-3">{props.icon}</div>
<p className="text-slate-500 text-xs font-bold uppercase tracking-wide">{props.label}</p>
<p className={`text-2xl md:text-3xl font-black mt-1 ${props.valueClassName ?? ""}`}>{props.value}</p>
</div>
);
}
...
try {
  ...
  const { data: available, error: availableError } = await supabase
    ...
    .is("washer_id", null)
    .is("partner_id", null)
    .in("status", ["pending", "confirmed"])   // ← LA SEULE LIGNE CHANGÉE
...
try {
  const { error } = await supabase
    ...
    .eq("id", orderId)
    .is("washer_id", null); // ✅ Protection anti race-condition

  if (error) throw error;

  toast.success("Mission acceptée ! 🎉");
  setActiveTab("active"); // ✅ Bascule auto sur "Mes missions"
  fetchWasherData();
} catch (error) {
...
<Navbar />
  ...
  <div className="pt-24 md:pt-32 px-4 max-w-7xl mx-auto pb-12">
    ...
    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 md:mb-8">
      <div>
        <h1 className="text-2xl md:text-3xl font-black mb-1 md:mb-2">Mon Dashboard Washer</h1>
        <p className="text-slate-600 text-sm md:text-base">Gérez vos missions et vos revenus</p>
      </div>

      {/* ✅ FIX MOBILE: boutons wrappables sur petits écrans */}
      <div className="flex flex-wrap items-center gap-2 md:gap-3">
        <button
          type="button"
          onClick={fetchWasherData}
          className="px-3 md:px-4 py-2.5 md:py-3 rounded-xl border bg-white hover:bg-slate-50 font-bold flex items-center gap-2 text-sm"
          disabled={refreshing}
          title="Rafraîchir"
        >
          <RefreshCcw size={16} className={refreshing ? "animate-spin" : ""} />
          <span className="hidden sm:inline">{refreshing ? "Actualisation..." : "Rafraîchir"}</span>
        </button>

        {notificationsEnabled ? (
          <div className="px-3 md:px-4 py-2 bg-green-50 border border-green-200 rounded-xl flex items-center gap-2">
            <Bell size={14} className="text-green-600" />
            <span className="text-xs md:text-sm font-bold text-green-700">Notifs ON</span>
          </div>
        ) : (
          <div className="px-3 md:px-4 py-2 bg-orange-50 border border-orange-200 rounded-xl flex items-center gap-2">
            <Bell size={14} className="text-orange-600" />
            <span className="text-xs md:text-sm font-bold text-orange-700">Notifs OFF</span>
          </div>
        )}
        ...
        <button
          ...
          className={`px-4 md:px-6 py-2.5 md:py-3 rounded-xl font-bold flex items-center gap-2 transition-all text-sm md:text-base ${
            isAvailable
              ? "bg-green-500 text-white hover:bg-green-600 shadow-lg"
              : "bg-slate-300 text-slate-600 hover:bg-slate-400"
          }`}
        >
          <div className={`w-2.5 h-2.5 rounded-full animate-pulse ${isAvailable ? "bg-white" : "bg-slate-500"}`} />
    ...
    {/* ALERTES */}
    {!isAvailable && (
      <div className="bg-orange-50 border-2 border-orange-200 rounded-xl p-4 mb-4 md:mb-6 flex items-center gap-3">
    ...
    {!notificationsEnabled && (
      <div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-4 mb-4 md:mb-6 flex items-center gap-3">
    ...
    {!stripeConnectStatus.completed ? (
      <div className="bg-gradient-to-r from-purple-500 to-pink-500 text-white p-6 md:p-8 rounded-3xl mb-6 md:mb-8 shadow-2xl">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="text-lg md:text-2xl font-black mb-2">Configurez vos virements automatiques</h3>
            ...
            <button
              ...
              className="px-6 md:px-8 py-3 md:py-4 bg-white text-purple-600 rounded-xl font-bold hover:bg-purple-50 transition shadow-lg flex items-center gap-2 text-sm md:text-base"
            >
              <ArrowRight size={18} />
      ...
      <div className="bg-green-50 border-2 border-green-200 p-5 md:p-6 rounded-2xl mb-6 md:mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-green-900 text-sm md:text-base">Compte bancaire connecté ✅</h3>
            <p className="text-xs md:text-sm text-green-700">Vos virements seront effectués chaque dimanche soir.</p>
          </div>
        </div>
      </div>
    )}
    ...
    {/* PREFERENCES */}
    {washerData && (
      <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-200 mb-6 md:mb-8">
        <div className="flex items-center gap-2 mb-5 md:mb-6">
          <Settings size={22} className="text-teal-600" />
          <h3 className="text-lg md:text-xl font-bold">⚙️ Mes préférences</h3>
        </div>
        ...
        <div className="space-y-5 md:space-y-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 text-slate-600">
              ...
              />
              <span className="text-sm">Uniquement ma ville ({washerData.city})</span>
            </div>
    ...
    {activeTab === "available" && (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        {availableMissions.map((mission) => (
          <div
            key={mission.id}
            className="bg-white rounded-2xl p-5 md:p-6 border hover:shadow-xl transition-all"
          >
            <div className="flex justify-between items-start mb-4">
              <div>
                <p className="text-xs text-slate-400 mb-1">Vous gagnez</p>
                <p className="font-bold text-2xl md:text-3xl text-teal-600">{commissionEarnings(mission.total_price)}€</p>
              </div>

              <span className={`px-2 py-1 rounded-full text-xs font-bold ${formulaBadgeClass(mission.formula)}`}>
                {formulaLabel(mission.formula)}
              </span>
            </div>
            ...
            <button
              ...
              className="w-full py-3 bg-gradient-to-r from-teal-600 to-teal-500 text-white rounded-xl font-bold hover:from-teal-500 hover:to-teal-400 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg text-sm md:text-base"
            >
              <Check size={18} />
              Accepter la mission
            </button>
        ...
        {availableMissions.length === 0 && (
          <div className="col-span-full text-center py-16 md:py-20 bg-white rounded-2xl border">
            ...
            <p className="text-xs text-slate-300 mt-4">Mise à jour auto toutes les 30s</p>
          </div>
        )}
      </div>
    )}
    ...
    {activeTab === "active" && (
      <div className="space-y-4">
        {activeMissions.map((mission) => (
          <div key={mission.id} className="bg-white rounded-2xl p-5 md:p-6 border hover:shadow-lg transition">
            <div className="flex flex-col md:flex-row justify-between items-start gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 mb-3 flex-wrap">
                  <p className="font-bold text-xl md:text-2xl text-teal-600">{commissionEarnings(mission.total_price)}€</p>
                  <span
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-slate-600">
                  ...
                  <p className="flex items-center gap-2">
                    <Package size={14} className="shrink-0 text-slate-400" />
                    {mission.weight} kg • {formulaLabel(mission.formula)}
                  </p>
                  <p className="flex items-center gap-2">
                    <Calendar size={14} className="shrink-0 text-slate-400" />
                    {formatDateFR(mission.pickup_date)}
              ...
              {/* ✅ FIX MOBILE: boutons full-width sur mobile */}
              <div className="flex flex-col gap-2 w-full md:w-auto">
                {mission.status === "assigned" && (
                  <button
                    type="button"
                    onClick={() => updateMissionStatus(mission.id, "in_progress")}
                    className="w-full md:w-auto px-6 py-3 bg-purple-600 text-white rounded-xl font-bold hover:bg-purple-500 whitespace-nowrap transition flex items-center justify-center gap-2 text-sm"
                  >
                    <Play size={16} />
                    Commencer
                  </button>
                ...
                {mission.status === "in_progress" && (
                  <button
                    ...
                    className="w-full md:w-auto px-6 py-3 bg-green-600 text-white rounded-xl font-bold hover:bg-green-500 whitespace-nowrap transition flex items-center justify-center gap-2 text-sm"
                  >
        ...
        {activeMissions.length === 0 && (
          <div className="text-center py-16 md:py-20 bg-white rounded-2xl border">
    ...
    {activeTab === "history" && (
      <div className="space-y-3 md:space-y-4">
        {historyMissions.map((mission) => (
          <div
            key={mission.id}
            className="bg-white rounded-2xl p-5 md:p-6 border flex flex-col md:flex-row justify-between items-start md:items-center gap-3 md:gap-4 hover:shadow-lg transition"
          >
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-3 mb-2 flex-wrap">
                <p className="font-bold text-lg md:text-xl text-green-600">+{commissionEarnings(mission.total_price)}€</p>
