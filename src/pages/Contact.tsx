...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Pour l'instant, on simule l'envoi (ou on pourrait le stocker en base si on créait une table)
    // On utilise le toast pour confirmer.
    setTimeout(() => {
        setLoading(false);
        setSent(true);
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-200 p-8">
            {!sent ? (
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <button disabled={loading} className="w-full py-4 bg-slate-900 text-white font-bold rounded-xl flex items-center justify-center gap-2 hover:bg-slate-800 transition">
