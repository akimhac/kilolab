...
export default function AdminDashboard() {
  ...
  const deactivatePartner = async (id: string) => {
    ...
    try {
      ...
      toast.success("✅ Désactivé", { id: toastId });
      fetchData();
    } catch (error: any) {
      toast.error("❌ Erreur: " + error.message, { id: toastId });
    }
        ...
        <div className="mb-8">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleLogout} className="bg-red-50 border border-red-200 text-red-600 px-4 py-2 rounded-lg text-sm font-bold hover:bg-red-100 flex items-center gap-2">
                <LogOut size={16} />
                Déconnexion
              </button>
            </div>
            ...
            {activeTab === "overview" && (
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div>
                    <h3 className="text-lg font-bold mb-4 flex items-center gap-2"><MapPin className="text-purple-600" size={20} />Top 5</h3>
                    <ResponsiveContainer width="100%" height={300}>
                          ...
                          <tr key={partner.id} className="border-b hover:bg-slate-50">
                            ...
                            <td className="py-3 px-4 text-right">
                              <div className="flex justify-end gap-2">
                                {partner.is_active ? (
                                  <button onClick={() => deactivatePartner(partner.id)} className="bg-white border border-red-200 text-red-600 px-3 py-2 rounded-lg text-xs font-bold hover:bg-red-50">Désactiver</button>
                                ) : (
                                  <>
                                    <button onClick={() => rejectPartner(partner)} className="bg-red-50 text-red-600 border border-red-200 px-3 py-2 rounded-lg text-xs font-bold hover:bg-red-100 inline-flex items-center gap-1"><XCircle size={14} />Refuser</button>
                                    <button onClick={() => approvePartner(partner.id)} className="bg-green-600 text-white px-3 py-2 rounded-lg text-xs font-bold hover:bg-green-700 shadow-sm inline-flex items-center gap-1"><CheckCircle size={14} />Accepter</button>
                                  </>
                                )}
                              </div>
                            </td>
