...
const STEPS = [
  {
    number: '01', title: 'Commandez en 2 min', subtitle: 'Depuis votre canape',
    description: 'Choisissez votre formule, indiquez votre adresse et selectionnez un creneau. Paiement securise en ligne.',
  ...
  {
    number: '02', title: 'On vient chercher', subtitle: 'Enlevement gratuit',
    description: 'Un washer verifie pres de chez vous vient collecter votre linge a l\'heure choisie. Pas besoin d\'etre present.',
  ...
  {
    number: '03', title: 'Lavage premium', subtitle: 'Pro, seche, plie',
    description: 'Votre linge est lave, seche et minutieusement plie. Produits premium, soin professionnel garanti.',
  ...
  {
    number: '04', title: 'Livre chez vous', subtitle: 'Propre et plie',
    description: 'Votre linge vous est retourne propre, seche et soigneusement plie. Suivez la livraison en temps reel.',
        ...
        <div className="max-w-6xl mx-auto px-6 w-full">
          <div className="flex flex-col lg:flex-row gap-12 lg:gap-20 items-center">
                  ...
                  <div key={i} className={`flex items-center gap-3 transition-all duration-300 cursor-pointer ${i === activeStep ? 'opacity-100' : 'opacity-30'}`}
                    ...
                    {i === activeStep && (
                      <span className="ml-auto flex items-center gap-1 text-xs font-bold" style={{ color: step.accent }}>
                        <span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: step.accent }} /> Actif
                      </span>
                    )}
                  </div>
                ))}
            ...
            <div className="lg:w-7/12 w-full">
              {STEPS.map((step, i) => (
                <div key={i} className={`transition-all duration-500 absolute lg:relative ${i === activeStep ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 translate-y-4 pointer-events-none absolute inset-0'}`}
                  style={{ position: i === activeStep ? 'relative' : 'absolute', top: 0, left: 0, right: 0 }}>
                  <div className="relative bg-[#0f1729] border border-white/10 rounded-3xl p-8 overflow-hidden shadow-2xl"
                    ...
                    <div className="relative z-10 flex items-center gap-6">
                      ...
                      <div className="relative flex-shrink-0 w-24 h-24">
                        ...
                        <div className={`relative w-24 h-24 rounded-2xl bg-gradient-to-br ${step.color} p-0.5 shadow-xl`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <span className={`text-xs font-black tracking-widest uppercase bg-gradient-to-r ${step.color} bg-clip-text text-transparent`}>ETAPE {step.number}</span>
                        <h3 className="text-2xl font-black text-white leading-tight mt-1 mb-1">{step.title}</h3>
